...
#homoginize dates
new_date <- c("2021-01-29","2021-03-13","2021-04-30")
old_date <- c("2021-01-28","2021-03-12","2021-04-29")

df_survival$Date <- as.character(df_survival$Date)

df_survival <- df_survival %>%
  mutate(Date = recode(Date, !!!setNames(new_date, old_date)))
...
df_survival %>%
  ...
  geom_bar(aes(fill = Feed_Group), position = "dodge2", stat = "identity", width=5)
...
V_P <- ggplot(df_fecundity, aes(x=Sample.ID,Embryo.Viable,fill=Group)) + 
  geom_col() +
    scale_color_brewer(palette="Dark2") +
...
A_V<-df_fecundity %>%
  filter(Sample.ID != "10C6")%>%
  group_by(Group) %>%
  summarize(Mean = mean(Embryo.Viable),
            Standard_Error =
              sd(Embryo.Viable)/sqrt(length(Embryo.Viable))) %>%
ggplot(., aes(x=Group,Mean,fill=Group)) + 
  geom_col() + geom_errorbar(aes(ymin=Mean-Standard_Error,
