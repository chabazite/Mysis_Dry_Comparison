...
# turn sum into percentage for easier comparison between groups
 SS <- df_sex_sort %>%
   mutate(Total = rowSums(df_sex_sort[4:6])) %>%
    pivot_longer(.,Female:Unknown, names_to = "sex",values_to = "Census") %>%
  group_by(Group,sex) %>%
  summarise(Census=sum(Census), Total=sum(Total),Percent_total=(Census/Total)*100) %>%
  ggplot(.,aes(x=sex,y=Percent_total,fill=Group))+geom_col(position="dodge2")+
  scale_color_brewer(palette="Dark2") +
  ...
  #saving png files for medium post
pngfile <- fs::path(knitr::fig_path(),  "analysis/EDA_Figures/Sex_sort_percent.png")
agg_png(pngfile, width = 60, height = 36, units = "cm", res = 300)
plot(SS)
invisible(dev.off())
knitr::include_graphics(pngfile)
...
for (i in 1:length(df_sex_mean$Group)){
 ...
 S_A <- ggplot(df_sex_mean,aes(x=sex,y=Mean,fill=Group)) + 
   ...
   geom_errorbar(df_sex_mean,
                 aes(ymin=Mean-Standard_Error,
