...
gemma_outlier2 <- subset(growth_gemma_outlier_df, growth_gemma_outlier_df$Mass_mg %in% boxplot(growth_gemma_outlier_df$Mass_mg ~ growth_gemma_outlier_df$Age_Category)$out)
...
gemma_mass_boxplot_outlier_df <- growth_gemma_outlier_df[gemma_outlier_loc,]


mysis_outlier_loc <- which(growth_mysis_outlier_df$Mass_mg %in% c(mysis_outliers$...1) && 
                             growth_mysis_outlier_df$Age_Category %in% c(mysis_outliers$Age_Category))

mysis_mass_boxplot_outlier_df <- growth_mysis_outlier_df[mysis_outlier_loc,]
...
gemma_length_outlier_loc <- which(growth_gemma_outlier_df$Length_mm %in% c(gemma_outliers_length$...1) & 
                             growth_gemma_outlier_df$Age_Category %in% c(gemma_outliers$Age_Category))

gemma_length_boxplot_outlier_df <- growth_gemma_outlier_df[gemma_length_outlier_loc,]


mysis_length_outlier_loc <- which(growth_mysis_outlier_df$Length_mm %in% c(mysis_outliers_length$...1) & 
                             growth_mysis_outlier_df$Age_Category %in% c(gemma_outliers$Age_Category))

mysis_length_boxplot_outlier_df <- growth_mysis_outlier_df[mysis_length_outlier_loc,]
...
ggplot(growth_mysis_outlier_tidy_df, aes(x=Outlier_Count,fill=Outliers_Metric ))+geom_bar()

ggplot(growth_mysis_outlier_tidy_df, aes(x=Outliers_Metric, fill=Outlier_Count ))+geom_bar()

ggplot(growth_mysis_outlier_tidy_df, aes(y=Mass_mg, x=Length_mm, color= Outlier_Count ))+geom_point()
...
ggplot(growth_mysis_nofactor_outlier_df, aes(y=Total_Outlier, x=Sample_ID)) + 
         geom_col()+scale_color_brewer(palette="Set1")
...
#Plotting only the outliers that recieved 1 method for both length and mass combined
ggplot(growth_mysis_nofactor_outlier_df, aes(x=Length_mm,y=Mass_mg, color=Total_Outlier==1)) + 
         geom_point()+scale_color_brewer(palette="Set1")

#plotting outliers that recieved one method of detection for mass
ggplot(growth_mysis_nofactor_outlier_df, aes(y=Mass_mg,x=Age_Category)) + 
         geom_boxplot()+scale_color_brewer(palette="Set1") + geom_point()+
  geom_point(data=. %>% filter(Sample_ID %in% mysis_mass_boxplot_outlier_df$Sample_ID),color="red")                                                                 
#plotting outliers that recieved one method of detection for length

ggplot(growth_mysis_nofactor_outlier_df, aes(y=Length_mm,x=Age_Category)) + 
         geom_boxplot()+scale_color_brewer(palette="Set1") + 
  geom_point(data=. %>% filter(Sample_ID %in% mysis_length_boxplot_outlier_df$Sample_ID),color="red")                                                                 
