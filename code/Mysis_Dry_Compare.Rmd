...
BP_M <- ggplot(df_growth,aes(y=Mass_mg, x=Age_Category)) + 
   geom_boxplot(lwd=2, 
               fill='grey', 
               aes(color=Feed_Group), 
               outlier.colour = NULL,
               outlier.size = 3) +
  scale_color_brewer(palette="Dark2") +
  labs(title="Mass of Surface Morph by Age & Feed Type", 
       subtitle = 'Box Plot' ,
...
BP_L <- ggplot(df_growth,aes(y=Length_mm, x=Age_Category)) + 
  geom_boxplot(lwd=2, 
               fill='grey', 
               aes(color=Feed_Group), 
               outlier.colour = NULL,
               outlier.size = 3) +
  scale_color_brewer(palette="Dark2") +
  labs(title='Length of Surface Morph by Age & Feed Type', 
       subtitle = "Box Plot",
...
#understanding Tanks and Age
df_growth %>%
  filter(Feed_Group == "Mysis") %>%
ggplot(., aes(y=Mass_mg, x=Age_Category,color=Age_Category)) + geom_boxplot(aes(color=Age_Category))+
 theme_minimal()+
  theme(
        legend.position = 'none',
    
  ) +
  facet_wrap(Tank~Feed_Group)

#looking at mass vs age through the lens of our different samples and groups

BP_MT <- ggplot(df_growth, aes(y=Mass_mg, x=Age_Category)) + 
  geom_boxplot(lwd=1,
               fill = 'grey',
               aes(color= Tank), 
               outlier.colour = NULL,
...
#Same graph but using Length

BP_LT <- ggplot(df_growth, aes(y=Length_mm, x=Age_Category)) + 
  geom_boxplot(lwd=1,
               fill = 'grey',
               aes(color= Tank), 
               outlier.colour = NULL,
...
#Creating a Scatter Plot of Length Mass Relationship with Feed Groups as colors
SP <- ggplot(df_growth, aes(x = Length_mm, y = Mass_mg, color = Feed_Group)) + 
  geom_point(size=2,alpha=0.7) +
  scale_color_brewer(palette="Dark2") +
  labs(title='Mass vs Length Relationship', 
       subtitle = 'Feed Groups',
...
#Creating a Scatter Plot of Length Mass Relationship with Age as colors
SP_Age <- ggplot(df_growth, aes(x = Length_mm, y = Mass_mg, color = Age_Category)) + 
  geom_point(size=2,alpha=0.7,aes(shape=Feed_Group)) +
...
```{r Print EDA Graphics}



#Density Mass by Category Plot
...
#Density Length by Category Plot
...
#Density Mass by Age and Category Plot
...
#Density Length by Age and Category Plot
...
#Printing Length Mass Relationship Scatter Plot
...
#tidy data, summarize for mean and standard error
df_sex_mean<-df_sex_sort %>%
...
#create a dataframe using aggregate of the total count for each group
Total<- aggregate(df_sex_mean$Mean, by=list(Group=df_sex_mean$Group), FUN=sum)
#using the total count to normalize mean and standard error, iterate through rows
#if the row is a mysis row, divide the mean and error by the mysis total
#otherwise divide by the gemma total.
for (i in 1:length(df_sex_mean$Group)){
    if (df_sex_mean$Group[i] == "Mysis") {
      df_sex_mean$Mean[i] <- df_sex_mean$Mean[i]/Total$x[2]
      df_sex_mean$Standard_Error[i] <- df_sex_mean$Standard_Error[i]/Total$x[2]
  } else {
     df_sex_mean$Mean[i] <- df_sex_mean$Mean[i]/Total$x[1]
     df_sex_mean$Standard_Error[i] <- df_sex_mean$Standard_Error[i]/Total$x[1]
  }
}

  
 S_A <- ggplot(df_sex_mean,aes(x=sex,y=Mean,fill=Group)) + 
...
S_T <- ggplot(df_survival, aes(x=Date, y=Census, group = Tank, color = Feed_Group)) +
  geom_line(size = 2) + geom_point(size=4) +
  scale_color_brewer(palette="Dark2") +
  labs(title='Fish Census per Sample Tank over Time', 
...
#plot the difference in means from one census event to the next
df_survival %>%
  group_by(Feed_Group,Date) %>%
  summarise(Census = mean(Census)) %>%
  summarise(Census_Dead = diff(Census))%>%
  ggplot(.,aes(y = Census_Dead, color=Feed_Group))+geom_bar()


df_fake <- data.frame(Date = as.Date(c("2020-10-15", "2020-12-05","2021-01-29",
                      "2021-03-13","2021-04-30","2021-07-06", "2020-10-15", "2020-12-05","2021-01-29",
                      "2021-03-13","2021-04-30","2021-07-06")),
                      Feed_Group =c("Gemma","Gemma","Gemma","Gemma","Gemma","Gemma",
                                "Mysis","Mysis","Mysis","Mysis","Mysis","Mysis"),
                      Census_Dead = c(0,6,1.25,.5,.5,.5,0,3.5,0.167,.167,.3,.45))

ggplot(df_fake,aes(x=Date,y=Census_Dead,fill=Feed_Group))+
  geom_col(position="dodge2")+ geom_line()+ scale_color_brewer(palette="Dark2") 
  

D_B<- ggplot(df_fake,aes(x=Date,y=Census_Dead,fill=Feed_Group))+
  geom_col(position="dodge2")+ geom_line(aes(y=Census_Dead)+ scale_color_brewer(palette="Dark2") 
  labs(title='Average Deaths over Time', 
       subtitle = 'Per Event',
...
df_fecundity<-read.xlsx("../data/Mysis_Pilot.xlsx", sheet="Exp 2",detectDates=T)
df_fecundity$Sample.ID <- factor(df_fecundity$Sample.ID)
df_fecundity$Group <- factor(df_fecundity$Group)
summary(df_fecundity)

ggplot(df_fecundity, aes(x=Sample.ID,Embryo.Viable,fill=Group)) + geom_col() +
    scale_color_brewer(palette="Dark2") +
  labs(title='Average Feed Group Census over Time', 
       subtitle = 'Standard Error for Comparison',
       y = "Fish Census", x = "Date") +
  ...
  labs(title="Significance Testing") + 
  labs( x="Groups", y = "Total Viable Embryos per breeding day") + 
  theme(
    panel.grid.major = element_blank(), 
    panel.grid.minor = element_blank(),
    axis.text=element_text(size=16, face="bold",angle = 90, hjust = 1), 
    axis.text.x=element_text(size=16,angle = 45, hjust = 1), 
    axis.title=element_text(size=18,face="bold"), 
    title=element_text(size=30,face="bold"), 
    plot.title = element_text(hjust = 0.5), 
    plot.margin = margin(t = 15, r = 15, b = 15, l = 15, unit = "pt")) +
  annotate(geom="text",x=1.5,y=3000,label="p-value= 0.005893",color="black",fontface="bold") + 
  annotate(geom="text",x=1,y=400,label="mean= 210.94",color="black",fontface="bold") + 
  annotate(geom="text",x=2,y=900,label="mean= 705.42",color="black",fontface="bold")
