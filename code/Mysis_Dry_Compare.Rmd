...
df_growth<-read.xlsx("H:/Data Science/Mysis/data/Mysis_Pilot.xlsx", 
                     sheet="Exp_1",detectDates=T)
df_survival<-read.xlsx("H:/Data Science/Mysis/data/Mysis_Pilot.xlsx", 
                       sheet="Survival Census (Progress)",detectDates=T)
df_sex_sort <-read.xlsx("H:/Data Science/Mysis/data/Mysis_Pilot.xlsx", 
...
ggplot(df_growth, aes(x=Mass_mg, color=Feed_Group))+geom_density(alpha=0.2,fill='grey')+facet_wrap(~Age_Category)

#Histogram Plot of Length for Ages and Groups
ggplot(df_growth, aes(x=Length_mm, color=Feed_Group))+geom_density(alpha=0.2,fill='grey')+facet_wrap(~Age_Category)

ggplot(df_growth, aes(x=Length_mm,y=Age_Category, fill=Age_Category))+geom_density_ridges()+
  theme_ridges()

#Growth Curves of Gemma and Mysis Mass vs Length
ggplot(df_growth, aes(x = Length_mm, y = Mass_mg, color = Feed_Group))+geom_point()+facet_wrap(~Feed_Group)+geom_smooth(method='lm',formula = y~poly(x,2,raw=TRUE),color='black')
