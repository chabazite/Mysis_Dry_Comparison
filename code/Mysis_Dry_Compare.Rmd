...
#Not seeing any significant differences using ANOVA, which confirms what we saw
# in the percentage comparison earlier
```
...
linear_Mod_Mysis <- df_survival %>%
  filter(Feed_Group == 'Mysis') %>%
  lm(Date.numeric ~ Census, data=.)

linear_Mod_Gemma <- df_survival %>%
...
#We will look to see if the regression lines of each summed group is statistically 
#different using ANCOVA


#differences in survival at every time period from the previous time period
