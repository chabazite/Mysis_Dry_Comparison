...
mysis_mass_outliers <- growth_mysis_outlier_df %>%
  ...
  summarize(outliers = boxplot.stats(Mass_mg)$out) %>%
...
for (i in 1:nrow(mysis_mass_outliers)) {
 ...
 mysis_mass_boxplot_outlier_df <- rbind(mysis_mass_boxplot_outlier_df, OneRowofOutlier)
}
...
for (i in 1:nrow(gemma_mass_outliers)) {
 ...
 gemma_mass_boxplot_outlier_df <- rbind(gemma_mass_boxplot_outlier_df, OneRowofOutlier)
}
...
for (AgeCategory in levels(growth_gemma_outlier_df$Age_Category)){
  ...
  gemma_cooksd_outlier_loc <- AgeGroup_df %>% 
                                filter(row_number() %in% names(influencial_Observations))

  gemma_mass_cooksd_outlier_df <- rbind(gemma_mass_cooksd_outlier_df, 
                                       gemma_cooksd_outlier_loc)
...
#Can I use apply() here pretty sure yes
...
#Add +1 to any method that detected the sample ID. 
...
mysis_length_outlier_method_df <-  subset(mysis_length_summary_outlier_df,
                                        select=-c(Date, Tank_Location, Feed_Group, Mass_mg)) 
...
ggplot(growth_mysis_outlier_df, aes(y=Length_mm, x=Tank_Location,color=Age_Category)) + geom_boxplot(aes(color=Age_Category))+
 theme_classic()

ggplot(growth_mysis_outlier_df, aes(y=Length_mm, x=Age_Category,color=Tank_Location)) + geom_boxplot(aes(color=Tank_Location))+
  theme_classic()

ggplot(growth_gemma_outlier_df, aes(y=Length_mm, x=Tank_Location,color=Age_Category)) + geom_boxplot(aes(color=Age_Category))+
 theme_classic()

ggplot(growth_gemma_outlier_df, aes(y=Length_mm, x=Age_Category,color=Tank_Location)) + geom_boxplot(aes(color=Tank_Location))+
  theme_classic()
